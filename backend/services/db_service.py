...
def save_posttest(nip: str, soal: list) -> dict:
    data = {
        ...
        "completed": False,
        "saved_jawaban": {},
        "adaptive_state": {"phase": "stage1", "checkpoint_done": False},
        "jalur": None,
        "kategori": None,
    }
...
def save_answer(nip: str, nomor: int, jawaban: str) -> dict:
    """Save a single answer to saved_jawaban (for lost connection recovery)."""
    posttest = get_posttest(nip)
    if not posttest:
        return {}
    saved = posttest.get("saved_jawaban") or {}
    saved[str(nomor)] = jawaban
    res = (
        supabase.table("posttest")
        .update({"saved_jawaban": saved})
        .eq("nip", nip)
        .execute()
    )
    return res.data[0] if res.data else {}
...
def update_adaptive_state(nip: str, state: dict) -> dict:
    """Update adaptive test state (phase, checkpoint info, etc.)."""
    res = (
        supabase.table("posttest")
        .update({"adaptive_state": state})
        .eq("nip", nip)
        .execute()
    )
    return res.data[0] if res.data else {}
...
def submit_posttest(nip: str, nilai: int, soal_updated: list, jalur: str = None, kategori: str = None) -> dict:
    from datetime import datetime, timezone
    update_data = {
        "nilai": nilai,
        "soal": soal_updated,
...
def update_posttest_soal(nip: str, soal: list, jalur: str = None) -> dict:
    """Update posttest soal (e.g., after checkpoint adds soal 26-30)."""
    update_data = {"soal": soal}
    if jalur:
        update_data["jalur"] = jalur
    res = (
        supabase.table("posttest")
...
def reset_posttest_for_retake(nip: str, new_attempts: int, soal_bersih: list) -> dict:
    try:
        ...
        print(f"Warning: some columns may not exist. {e}")
        res = (
            supabase.table("posttest")
            .update({
                "completed": False,
